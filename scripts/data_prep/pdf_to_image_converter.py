...
def convert_pdf_to_images(pdf_path, output_folder, asset_name, suspicious_folder, log_entries):
    ...
    for i, image in enumerate(images):
        ...
        if width < MIN_IMAGE_WIDTH or height < MIN_IMAGE_HEIGHT:
            ...
            print(f"⚠️ Saved suspicious image: {small_img_path.name}")
            continue

        # Save high-resolution version
        image.save(full_output)
        print(f"🖼️ Saved full-res: {full_output.name} | Size: {image.size}")
        ...
        if width * height > MAX_SAFE_PIXELS:
            ...
            print(f"⚙️ Saved downscaled ML version: {ml_output.name} | Size: {image_ml.size}")
        else:
...
def convert_pdfs_for_assets(base_data_path, asset_names):
    ...
    for asset in asset_names:
        ...
        if not pdf_files:
            print(f"⚠️ No PDFs found for asset {asset} in {input_folder}")
            continue
