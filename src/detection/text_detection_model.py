...
training_dir = os.path.join(base_dataset_dir, 'Training', 'Patches')
validation_dir = os.path.join(base_dataset_dir, 'Validation', 'Patches')

# Output directories
training_out_dir = os.path.join(base_dataset_dir, 'Training', 'Text_Processed')
validation_out_dir = os.path.join(base_dataset_dir, 'Validation', 'Text_Processed')
os.makedirs(training_out_dir, exist_ok=True)
os.makedirs(validation_out_dir, exist_ok=True)

# EAST model path 
east_model_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'models', 'frozen_east_text_detection.pb'))
...
for image_path in training_image_paths:
    ...
    output_path = os.path.join(training_out_dir, os.path.basename(image_path))
    cv2.imwrite(output_path, processed_image)

# Process and save validation images
for image_path in validation_image_paths:
    processed_image = process_image(image_path, net)
    output_path = os.path.join(validation_out_dir, os.path.basename(image_path))
    cv2.imwrite(output_path, processed_image)